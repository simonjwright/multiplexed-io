...
package body SPI2.BARO is
   ...
   begin
      --  Reset the MS5611
      Internal.Write_SPI (Internal.BARO, (0 => Reset'Enum_Rep));
      --  MS5611 needs 2.8 ms
      delay until Ada.Real_Time.Clock + Ada.Real_Time.Milliseconds (3);
      ...
      for J in 1 .. 6 loop
         ...
         begin
            Internal.Command_SPI
              (Internal.BARO,
               (0 => Prom_Read_0'Enum_Rep + 2 * Interfaces.Unsigned_8 (J)),
               Raw);
      ...
      loop
         ...
         Internal.Write_SPI (Internal.BARO,
                             (0 => Convert_Temperature_4096'Enum_Rep));

         --  max conversion time is 9.04 ms w/ precision 4096
         delay until Ada.Real_Time.Clock + Ada.Real_Time.Milliseconds (10);

         --  retrieve the result
         Internal.Command_SPI (Internal.BARO, (0 => ADC_Read'Enum_Rep), Raw);

         D2 := Float ((Integer (Raw (0)) * 256
                         + Integer (Raw (1))) * 256
                        + Integer (Raw (2)));
         DT := D2 - (Coefficients (T_REF) * 2.0 ** 8);
         TEMP := 2000.0 + DT * Coefficients (TEMPSENS) / 2.0 ** 23;

         Ada.Text_IO.Put_Line ("temp: " & Integer (TEMP)'Img);

         --  process pressure
         --  start the conversion
         Internal.Write_SPI (Internal.BARO,
                             (0 => Convert_Pressure_4096'Enum_Rep));
         ...
         Ada.Text_IO.Put_Line ("prss: " & Integer (P)'Img);
